...
@test "preview (whole) shows your edits and leaves the review untouched" {
	...
	[[ "$output" == *"a.txt"* ]]
	# and finish-review still works afterwards — preview banked/committed nothing
	git finish-review
...
@test "preview (step) merges a banked edit whose context the PR later changed" {
	...
	git review-pr feature/merge --step            # step 1 (cg1, g.txt = G1..G7)
	printf 'G1\nG2\nG3\nG4-MINE\nG5\nG6\nG7\n' >g.txt
...
@test "preview's overlap note matches what finish-review actually hits" {
	# The note is only honest if finish-review really conflicts in the same state.
	setup_conflict_pr
	git review-preview >/dev/null 2>&1
	# preview left the state intact, so finish-review runs from the same place
...
@test "preview refuses while finish-review is mid-conflict" {
	setup_conflict_pr
	# drive finish-review into its conflict state (markers in the tree, reviewresume set)
	run git finish-review
	[ "$status" -ne 0 ]
