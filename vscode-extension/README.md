...
| **Walkthrough: Init / Update / Build** | Empty state, under *Walkthrough* (this is the *author* flow); also the palette. Reads *Update* once a walkthrough exists, because that is what the verb does then: it keeps every entry whose file is still in range and adds the ones that are new | `git review walkthrough init` / `build` |
