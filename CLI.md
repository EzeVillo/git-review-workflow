# git-review-workflow CLI reference

Complete command-line reference for git-review-workflow.

[Back to the README](README.md) · [Español](CLI.es.md)
...
Run the visual terminal interface from inside a repository:

```sh
git review ui
```

On network mounts whose file notifications are unreliable, set a minimum
refresh interval. Every normal refresh resets that floor, so this does not add
polling while notifications are arriving:
...
Homebrew configures completion automatically. For a manual installation, tell
your shell to load the matching file on start. Replace
`/path/to/git-review-workflow` with where you downloaded the project.
...
Then open a new terminal. Typing `git review ` and pressing **Tab** offers the
verbs; `git review start ` offers your branch names.
