...
@test "review forget --delta <branch> forgets both the remote and the local marker" {
	# A branch reviewed both ways carries two markers in disjoint config sections.
	# A single forget must clear both (the two any=1 arms), not just the remote one.
	mark feature/x
	git config "reviewworkflowlocal.feature/x.reviewed" "$(git rev-parse origin/feature/x)"
...
@test "review forget --delta rejects combining a branch with --stale" {
	mark feature/x
	run git review forget --delta feature/x --stale
	[ "$status" -ne 0 ]
	[[ "$output" == *"only one of"* ]]
	# the conflict is caught before any marker is touched
	[ -n "$(git config reviewworkflow.feature/x.reviewed)" ]
}
...
# ── mode selection (--delta vs --saved) ─────────────────────────────────────────

@test "review forget with no mode prints usage and exits 1" {
	mark feature/x
	run git review forget feature/x
	[ "$status" -eq 1 ]
	[[ "$output" == *"usage: git review forget"* ]]
	# nothing forgotten without a mode
	[ -n "$(git config reviewworkflow.feature/x.reviewed)" ]
}
