...
**Tests**: la spec pide "controlá los errores de la mejor manera" y `CLAUDE.md`
manda asserts fuertes para todo comportamiento observable; `003`/`004` fijaron el
precedente test-first para cambios de porcelain y se mantiene acá — se incluyen
en cada historia.

**Organization**: por historia de usuario, en el orden de prioridad de la spec
(P1 × 3, P2 × 2, P3 × 1). US1 es el MVP: sin ella ninguna otra historia tiene una
review que abrir. US2 y US3 dependen de que exista una review activa (que US1
crea) pero son independientes entre sí. US4 depende del contrato que US3
...
Proyecto único, dos subárboles: `bin/` (CLI, shell POSIX, tests en `tests/` vía
Docker) y `vscode-extension/src` (TypeScript, tests en `vscode-extension/test/`).
Ningún archivo cruza esa línea.

---

## Phase 1: Setup

**Purpose**: línea base verde antes de tocar código que varias historias
comparten.
...
- [ ] T001 Correr `./tests/run-docker.sh` completo y `npm test --prefix
  ...
  partida limpio antes de invertir ningún test.

---

## Phase 2: Foundational (Blocking Prerequisites)

**Purpose**: la infraestructura que **toda** acción mutante nueva (US1-US5)
necesita — research.md Decisiones 5, 6, 7 y 8. Sin esto, la primera historia que
la implemente tendría que resolverla a mitad de su propio trabajo, y las
siguientes cuatro la reimplementarían con matices distintos.

**⚠️ CRITICAL**: T002-T009 bloquean la implementación de todas las historias.
Los tests de cada historia (que deben escribirse y fallar primero) no dependen
de esta fase, salvo cuando se indica.

### Tests foundational ⚠️

- [ ] T002 [P] En `vscode-extension/test/unit/mutationLock.spec.ts`, agregar el
  caso: una segunda `run()` mientras la primera está en vuelo invoca un callback
  `onDiscarded` (o dispara `onDidChangeBusy` con una razón), además de devolver
  `undefined` como hoy (research.md Decisión 7, FR-036).
- [ ] T003 [P] Crear `vscode-extension/test/unit/staleGuard.spec.ts`: un
  ...
  `tip` o `situation` distintos, rechaza con el motivo (research.md Decisión 8,
  FR-038).
- [ ] T004 [P] Crear `vscode-extension/test/unit/invokeClass.spec.ts`: una
  función pura `timeoutForClass(verb, args)` devuelve 15000 para lecturas
  (`status`, `list`, `config`, `--why`, `--version`), 120000 para mutaciones
  locales (`finish`, `save`, `abort`, `continue`, `next`, `prev`), 300000 para
  `start` (research.md Decisión 6).

### Implementación foundational

- [ ] T005 [P] En `vscode-extension/src/review/mutationLock.ts`, agregar
  `onDidDiscard`/razón de descarte a `MutationLock.run()`: cuando `busy` ya es
  `true`, además de devolver `undefined` notifica a los listeners con el motivo
  ("otra operación está en curso"), para que quien la dispare desde la paleta o
...
- [ ] T006 [P] Crear `vscode-extension/src/review/staleGuard.ts`: tipo
  `StateToken = {branch?: string; tip?: string; situation: Situation}`, función
  `captureToken(state: ReviewState): StateToken` y `tokenStillValid(token,
  state): boolean` (data-model.md § `StateToken`). Sin `vscode`, función pura.
  Depende de T003.
- [ ] T007 En `vscode-extension/src/cli/invoke.ts`, agregar
  `timeoutForClass(verb: string): number` (research.md Decisión 6) y usarla como
  default de `timeoutMs` en `invokeGitReview` cuando el llamador no pasa uno
  explícito. Depende de T004.
...
- [ ] T008a En `vscode-extension/src/cli/invoke.ts`, agregar a `InvokeOptions` un
  flag `network?: boolean` que, cuando está en `true`, agrega al `env` del
  proceso hijo `GIT_TERMINAL_PROMPT=0` y apunta `GIT_ASKPASS`/`SSH_ASKPASS` al
  script de T008 (research.md Decisión 5). Sin uso todavía — lo consume T024
  (`start`). Depende de T008.
- [ ] T009 En `specs/002-extension-vscode/contracts/cli-invocation.md`, agregar
  al encabezado un puntero explícito: *"Enmendado por
  [`005-ciclo-review-panel/contracts/cli-invocation.md`](../../005-ciclo-review-panel/contracts/cli-invocation.md),
  que es el que rige a partir de esa feature."* (FR-001 — no pueden convivir dos
  listas vigentes que se contradigan).

**Checkpoint**: con T005-T009 hechos, cualquier historia puede implementar su
acción mutante sobre la misma base.

---

## Phase 3: User Story 1 - Empezar a revisar sin saber el comando (Priority: P1) 🎯 MVP

**Goal**: desde el estado vacío del panel, el revisor elige rama, forma de
lectura y (si hace falta) fija la base, y queda con una review activa
idéntica a la que dejaría el comando equivalente.

**Independent Test**: en un repositorio sin review, iniciar una desde el panel
y verificar contra `git review status` que la rama, el modo y la posición
coinciden con los del comando equivalente (quickstart.md § 2).
...
- [ ] T010 [P] [US1] Crear `tests/config.bats`: forma humana de
  ...
  `base` si no está configurada); `git review config base` (imprime el valor o
  ...
  sin existir — el efecto colateral no ocurrió). **Las mismas formas sobre
  `remote`**: FR-010b exige que la escritura cubra la misma configuración que el
  reporte, así que testear sólo `base` dejaría la mitad sin red. Incluir el caso
  de leer `remote` sin configurar, que devuelve el default `origin` y no vacío.
- [ ] T011 [P] [US1] En `tests/config.bats`, agregar: fuera de un repositorio
  git, exit `1` con el mismo diagnóstico que los demás verbos; con review
  ...
  tratarla como opción (mismo caso que `tests/range.bats` ya cubre para `start`).
- [ ] T012 [P] [US1] Crear `tests/config-porcelain.bats`: `--porcelain` en un
  repositorio recién clonado sin configurar emite sólo `config	remote	origin`
  (`base` omitida, no vacía); con `base` configurada, emite las dos líneas.
- [ ] T013 [P] [US1] En `tests/config-porcelain.bats`, agregar: sobre el sandbox
  ...
  `review-fixes/*` ni `<remote>/HEAD`; `current=1` sólo en la rama donde está
...
- [ ] T014 [P] [US1] En `tests/config-porcelain.bats`, agregar: una rama local
  ...
  citar, igual que un path — mismo criterio que `porcelain-bytes.bats` ya aplica
  a paths. Confirmar además que el costo es constante: con 30 ramas de más
  creadas en el fixture, `--porcelain` sigue completando sin que el test tenga
  que esperar más que con el fixture base (guard de regresión, no un benchmark
  estricto).
- [ ] T015 [P] [US1] Crear `vscode-extension/test/unit/configPorcelain.spec.ts`:
  ...
  `status`); un `candidate` duplicado (misma rama, dos orígenes) produce dos
  entradas, no una fusionada.
- [ ] T016 [P] [US1] Crear `vscode-extension/test/unit/reviewIntent.spec.ts`:
  la función que traduce `ReviewIntent` a argv (data-model.md § `ReviewIntent`)
  — `layout: "auto"` no agrega flags, `"step"` agrega `--step`, `"no-walk"`
  agrega `--no-walk`; `source: "local"`/`"offline"` agregan `--local`/
  `--offline`; `range: "delta"` agrega `--delta`; nunca agrega `--base`, `--from`
  ni el `<base>` posicional (contracts/cli-invocation.md § `start`, tabla de
  argumentos permitidos). **Y el caso de U1**: una rama llamada `-foo` produce
  `[..., "--", "-foo"]`, con el `--` inmediatamente antes del nombre y después
  de todos los flags; el nombre nunca sale en una posición donde la CLI pudiera
  leerlo como opción.

### Implementación para US1

- [ ] T017 [US1] En `bin/git-review-lib.sh`, agregar `candidate_branches`: un
  único `git for-each-ref --format='%(refname)'` sobre `refs/heads/` y
  `refs/remotes/<remote>/`, filtrando en shell los namespaces `review/`,
  `review-saved/`, `review-fixes/` y `<remote>/HEAD`, y emitiendo
  `nombre<TAB>origin<TAB>current` por línea (research.md Decisión 3 — procesos
  constantes, no uno por rama). Depende de T013/T014 (deben fallar antes).
- [ ] T018 [US1] Crear `bin/git-review-verbs/config`: gramática `git review
  ...
  (`bin/git-review-verbs/start:92-109`): sin él, un valor que empieza con guion
  —un nombre de rama legal— se leería como opción. Claves válidas
  `base`→`reviewworkflow.base`, `remote`→`reviewworkflow.remote` (default
  `origin` si no está seteada). Exit `1` sobre clave desconocida, uso inválido o
...
- [ ] T019 [US1] En `bin/git-review-verbs/config`, la forma `--porcelain
  ...
  siempre, `base` si está), `candidate` (vía `candidate_branches`), y `delta`
  (sólo si se pasó `<rama>` y `reviewworkflow.<rama>.reviewed` o
  `reviewworkflowlocal.<rama>.reviewed` existe — mismo par de claves que lee
...
- [ ] T021 [US1] `shellcheck` limpio sobre `bin/git-review-verbs/config`,
  `bin/git-review-lib.sh` y `bin/git-review` (vía Docker). `./tests/run-docker.sh
  config.bats config-porcelain.bats`: en verde.
- [ ] T022 [P] [US1] Crear `vscode-extension/src/cli/configPorcelain.ts`:
  parser de línea a línea con el mismo tokenizador de `porcelain.ts` (split por
...
- [ ] T022a [US1] En `vscode-extension/src/review/state.ts`, poblar
  `ReviewState` con el reporte de configuración (`EffectiveConfig` + las
  ...
  una situación, deja el estado vacío como estaba. No se invoca con review
  activa — nada de lo que reporta se dibuja ahí, y agregar un proceso por
  refresco iría contra el costo que `002` acotó. Es el productor del dato que
...
- [ ] T023 [P] [US1] Crear `vscode-extension/src/review/reviewIntent.ts`: tipo
  ...
  string[]` — sólo los argumentos enumerados en
  contracts/cli-invocation.md § `start`, **incluido el `--` que precede
  siempre al nombre de rama** (U1: sin él, una rama llamada `-foo` se
  interpretaría como flag; es el idiom de git que el verbo ya soporta,
...
- [ ] T024 [US1] Crear `vscode-extension/src/commands/startReview.ts`:
  ...
  Decisión 9) — (1) `config --porcelain` con `options.network = false` para leer
  candidatas y config, con la rama actual primera y filtro incremental; (2) tres
  ítems con descripción (Automático / Commit por commit / Ignorar el
  walkthrough); (3) confirmación con la frase resumen (FR-017); (4) invoca
...
- [ ] T024a [US1] En `vscode-extension/src/commands/startReview.ts`, clasificar
  el fallo de `start` en las tres categorías que SC-007 exige diferenciar y
  presentar cada una distinto: **credenciales** (ofrece *Run in Terminal*, que
  manda el comando exacto a una terminal integrada donde sí hay quién conteste),
  **red** (el remoto no responde; sin escape a terminal, que no resolvería
  nada), y **repositorio** (working tree sucio, rama inexistente, review ya
  existente: el diagnóstico de la CLI y nada más). La clasificación mira el
  `stderr` **de git** que el verbo propaga, no la salida del verbo — la
  frontera está fijada en contracts/cli-invocation.md § "Clasificar no es
  parsear". Si la distinción entre las dos primeras no resulta fiable
  (research.md Decisión 5, riesgo registrado), colapsarlas en una y ofrecer el
  escape ante cualquier fallo de red: más ruidoso, nunca engañoso — y
...
- [ ] T025 [US1] Crear `vscode-extension/src/commands/setBase.ts`: `QuickPick`
  sobre las `candidate` del último reporte de `config --porcelain`, invoca
  `git review config base <rama>` al elegir. Invocable standalone (desde el
  estado vacío) y como paso del asistente de T024. Depende de T022.
- [ ] T026 [US1] En `vscode-extension/src/views/panelHtml.ts`, función
  ...
  reporte de `config` (obtenido al construir el estado vacío) no trae `base`, un
  párrafo adicional lo explica con un botón `Set the base branch` que dispara
  `gitReview.setBase` — no bloquea el asistente, que ya lo resuelve inline
  (FR-010/FR-010a).
- [ ] T027 [US1] En `vscode-extension/src/extension.ts`, registrar los comandos
  ...
  compartido (T005) y a `StaleGuard` no aplica acá (sin confirmación previa: el
  asistente entero es la decisión). En `vscode-extension/package.json`, agregar
  ambos a `contributes.commands` y a `commandPalette` con
  `"when": "gitReview.situation == no-review"` para `startReview`, sin
  restricción para `setBase`.
...
- [ ] T028 [US1] Crear `vscode-extension/test/integration/start-review.spec.ts`:
  ...
  actual, automático), confirmar, y afirmar contra `git review status
  --porcelain` invocado directo que la review resultante coincide con la que
  dejaría `git review start` a mano. Repetir con `--step` y `--no-walk`.
  Depende de T029.
...
**Checkpoint**: desde el estado vacío se llega a una review activa sin escribir
ningún comando, en cualquiera de las tres formas de lectura, y la base se puede
...
que el repositorio volvió al estado previo y el panel al estado vacío
(quickstart.md § 5).

**Depende de**: que exista una review activa que cancelar — la crea T024 (US1)
o el usuario a mano; el comando en sí no depende de ningún artefacto de US1
...
- [ ] T031 [P] [US2] Crear
  ...
  confirmación simulada, y afirmar que `git branch --list 'review/*'` ya no
  incluye esa rama y que `HEAD` volvió a la rama de origen (comparado contra el
  `reviewreturn` que la CLI registró al iniciar).
- [ ] T032 [P] [US2] En el mismo archivo, afirmar que descartar la confirmación
  (simulando que el `QuickPick`/`showWarningMessage` devuelve `undefined`) no
  invoca `abort` — verificado con un spy sobre `invokeGitReview` o revisando que
...
- [ ] T033 [US2] Crear `vscode-extension/src/commands/abortReview.ts`, con el
  ...
  lock, con `detail` que dice explícitamente "your uncommitted edits will be
  ...
  diálogo y lo revalida antes de invocar (FR-038): si no coincide, no invoca y
...
- [ ] T034 [US2] En `vscode-extension/src/extension.ts`, registrar
  ...
  botón en la barra del panel (`view/title` o dentro del webview, siguiendo el
  patrón de `refresh`).
- [ ] T035 [US2] En `vscode-extension/src/views/panelHtml.ts`, agregar el botón
...
**Goal**: cerrar la review desde el panel, eligiendo dónde quedan las
...
que las ediciones quedaron donde corresponde (quickstart.md § 3).
...
> ningún estado de cierre.**

- [ ] T038 [P] [US3] Crear `tests/finish-state.bats`: helper para construir el
  fixture de conflicto — modo `--step`, bancar una edición en un commit
  ...
  para que quede trabado (research.md Decisión 13, mismo mecanismo que
  `bin/git-review-verbs/finish:406-426`). Este helper lo reusan T037 y
  T039-T041, así que va primero.
- [ ] T037 [US3] En `tests/status-porcelain.bats`, agregar: sobre una rama
  `review/*` con un cierre trabado por conflicto (fixture de T038),
  `status --porcelain` sale con exit `0` y emite una línea `finish	conflict`,
  además del `state` de siempre. Afirmar también que sobre una review sin
  ningún cierre en curso el registro **no** aparece. Depende de T038.
- [ ] T039 [P] [US3] En `tests/finish-state.bats`, agregar: sobre un cierre
  **completo** (sin `--onto-source`), `git review list --porcelain` emite
  `finish	review/<src>	pending` para esa rama; con `--onto-source`, también
  (verificando que el `branch` reportado sigue siendo `review/<src>`, no la
  rama de destino). Sin ningún cierre pendiente, ninguna fila `finish`. **Y el
  segundo estado**: sobre el fixture de conflicto (T038), estando parado en
  otra rama, `list --porcelain` emite `finish	review/<src>	conflict` — el
  contrato dice que en `list` aparecen los dos estados, y `conflict` visto
  desde afuera es el caso que sólo `list` puede reportar.
- [ ] T040 [P] [US3] En `tests/finish-state.bats`, agregar: tras resolver el
  conflicto (aplicar la resolución al working tree) y correr `git review finish
  --resume`, el registro `finish` desaparece de `status --porcelain` de ahí en
  más. Tras `git review finish --abort` sobre un cierre `pending`, el registro
...
- [ ] T041 [P] [US3] En `tests/finish-state.bats`, agregar: una CLI que no
  ...
  contra un fixture sin cierre) no cambia ningún registro existente — es
...
- [ ] T042 [P] [US3] Extender `vscode-extension/test/unit/state.spec.ts`, que
  ya es el dueño de la derivación de `Situation`: con un `finish
  ...
  de los dos, el comportamiento de `002`/`003`/`004` no cambia (data-model.md §
  `Situation`).
- [ ] T043 [P] [US3] Extender `vscode-extension/test/unit/panelModel.spec.ts`:
  ...
  (o expone un flag `navigationLocked: true` — FR-027) aunque el `state` tenga
  cursor; con `finish-pending`, el inventario del estado vacío incluye la fila
  del cierre pendiente en vez de mostrarse vacío.

### Implementación para US3

- [ ] T044 [US3] En `bin/git-review-verbs/status`, forma `--porcelain`: después
  de emitir `state`, si `branch.$cur.reviewundohead` está seteado (un finish
  dejó un punto de undo sin resolver) **y** `branch.$cur.reviewresume` vale
...
- [ ] T045 [US3] En `bin/git-review-verbs/list`, forma `--porcelain`: por cada
  ...
  igual que en T044 — inmediatamente después de su registro `branch`. Depende de
  T039 (debe fallar antes).
- [ ] T045a [P] [US3] En `tests/finish-state.bats`, cubrir el campo `onto` en
  los dos verbos: un cierre con `--onto-source` reporta `1` y uno sin él `0`,
...
- [ ] T048 [US3] En `vscode-extension/src/review/situation.ts`, agregar
  ...
  `status` salió `2` y el inventario trae **al menos una** fila `finish …
  pending`, `situation = "finish-pending"`. Nada de "la rama previamente
  conocida": la extensión no tiene esa memoria después de reiniciarse, y el
  contrato no la pide — el inventario es autosuficiente, que es lo que hace
  cumplible SC-008. La precedencia es la de data-model.md § `Situation`
...
- [ ] T049 [US3] En `vscode-extension/src/views/panelModel.ts`,
  ...
  (FR-027); con `"finish-pending"`, `reviews` sigue poblándose desde
...
- [ ] T050 [US3] Crear `vscode-extension/src/commands/finishReview.ts`:
  `QuickPick` de dos ítems con descripción ("A separate branch" /
  "Onto the PR branch itself", FR-018) — sin casilla; captura `StateToken`
  ...
  lo que pase. **"No había ediciones que extraer" se deriva del estado
  ...
  — la CLI deshace su propio punto de undo en ese caso
  (`bin/git-review-verbs/finish:446-451`), así que la ausencia del registro es
  la señal, y es la misma que el contrato ya expone. Se informa como resultado
  normal, no como error (FR-019). Depende de T023 (patrón de `StaleGuard`),
...
- [ ] T050a [US3] En `vscode-extension/test/unit/` (junto a los tests del
  ...
  a un `finish` con exit `0` **sin** registro `finish pending`, la función que
  decide el mensaje devuelve "sin ediciones"; con el registro presente,
  devuelve "cierre pendiente". La función es pura y no recibe `stdout` ni
  `stderr` de ningún verbo — que es lo que hace verificable que no se parsea
  nada. Escribir antes de T050.
- [ ] T051 [US3] En `vscode-extension/src/views/panelHtml.ts`: caso
  `"finish-conflict"` en el estado de review — banner que explica el cierre
  trabado, sin los controles de navegación (usa `navigationLocked`), con los
  botones que US4 agrega (T057-T058); caso `"finish-pending"` dentro de
  `renderEmptyState` — encabeza el inventario con el cierre pendiente en vez de
...
- [ ] T052 [US3] En `vscode-extension/src/extension.ts`, registrar
  ...
  `commandPalette` con `"when": "gitReview.situation == review"`, botón en la
  barra del panel.
- [ ] T053 [P] [US3] Crear `vscode-extension/test/integration/finish-review.spec.ts`:
  ...
  ubicación, y afirmar contra `git review status --porcelain`/`list --porcelain`
  ...
  **Y FR-030**: descartar el `QuickPick` de ubicación no invoca `finish` — la
  review sigue activa y sin cierre pendiente, verificado por el efecto sobre
  git y no sólo por el retorno del comando.
- [ ] T054 [US3] `npm run compile`, `test:unit`, `test:integration` en verde.

**Checkpoint**: cerrar una review desde el panel dice dónde quedaron las
ediciones y el panel deja de mentir sobre "no hay ninguna review" cuando hay un
cierre sin resolver — aunque todavía no se pueda actuar sobre ese cierre (US4).

---

## Phase 6: User Story 4 - Deshacer un cierre, o destrabar uno que quedó a mitad (Priority: P2)

**Goal**: sobre los dos estados que US3 hizo visibles, el panel ofrece la
acción que corresponde a cada uno.

**Independent Test**: producir cada estado de cierre y verificar que el panel
ofrece exactamente su salida, y que ésta deja el repositorio como lo dejaría el
comando equivalente (quickstart.md § 3, sección "El cierre trabado").

**Depende de**: T044-T049 (US3) — sin el registro `finish` en el contrato, no
...
- [ ] T055 [P] [US4] En `vscode-extension/test/integration/finish-review.spec.ts`
  ...
  existiendo con el commit nuevo), y sólo entonces la segunda confirmación con
...
- [ ] T056 [P] [US4] En el mismo archivo: sobre el fixture de conflicto (T038),
  ...
  **Y el caso que motivó exponer `onto`**: un cierre trabado que había empezado
  con `--onto-source`, continuado después de recargar la ventana del editor
...
- [ ] T057 [US4] Extender `vscode-extension/src/commands/finishReview.ts` (o
  ...
  confirmación previa, así que la ventana de premisa caduca existe igual que en
  `finish`, `save` y `abort`); confirmación (FR-029) → invoca `git review finish
  --abort`. Si falla, el `stderr` de la CLI se muestra y se ofrece una
  **segunda** confirmación, visualmente distinta (texto que nombra el trabajo
  que se perdería, tomado del `stderr`) → sólo si se acepta, invoca `git review
  finish --abort --force` (FR-021 — nunca automático, nunca la misma
...
- [ ] T058 [US4] En el mismo módulo, la acción de continuar un cierre trabado:
  ...
  — **nunca de memoria del comando**: el editor se reinicia y ahí el resume
  mandaría las ediciones a un lugar distinto del que el usuario eligió, en
...
asimétrico que la CLI ya aplica.
...
existe y retomable con la acción que ya existe desde `002`.
...
ediciones volvieron (quickstart.md § 4).

**Depende de**: sólo de la Fase 2 (foundational) y de una review activa.
...
- [ ] T062 [P] [US5] Crear
  ...
  con su modo y posición. Repetir en modo `step` con ediciones en más de un
...
`continue` ya existía desde `002`, `save` se agrega acá.

---

## Phase 8: User Story 6 - Elegir qué se compara y de dónde sale (Priority: P3)
...
del comando equivalente (quickstart.md § 2, variantes).

**Depende de**: T017-T030 (US1) — extiende el mismo asistente y el mismo verbo
...
- [ ] T077 [P] Verificar a mano que el entorno no interactivo (T008/T008a) hace
  fallar rápido un `git fetch` contra un remoto que pide credenciales, en vez de
  colgarse hasta el timeout (quickstart.md § 6). No hay remoto autenticado en
  CI, así que la parte automatizable es el test de que el entorno **se pasa**;
  ésta es la mitad que sólo se comprueba a mano (research.md Decisión 13). Es
  también la validación que confirma o refuta el riesgo registrado en la
  Decisión 5, y de la que depende si T024a mantiene tres categorías o dos.
- [ ] T078 Ejecutar quickstart.md § 6 ("Demora") contra un repositorio clonado
  con historia grande: confirmar que el progreso es visible, el editor queda
  usable, y ningún control de la extensión es operable mientras `start` corre.
- [ ] T079 Ejecutar quickstart.md § 7 ("Estado cambiado por fuera"): abrir la
  confirmación de `finishReview`, correr `git review abort` en una terminal
...
- [ ] T080 Actualizar el mínimo de CLI a `0.4.0` en
  ...
  quickstart.md § 8 contra una CLI `0.3.x` real y confirmar el comportamiento
  **real**: la sesión entera entra en `cli-outdated` con el aviso de
  actualización — el panel no lee ni navega, exactamente como le pasó a `0.2.x`
  cuando `002` subió el mínimo a `0.3.0`. SC-009 se cumple así (el panel no
...
- [ ] T084 Verificar (no asumir) `docs/index.html` contra sus cuatro
  superficies duplicadas: la tabla comparativa no cambia; los métodos de
  instalación no cambian; los comandos de los ejemplos —si alguno usa
  `reviewworkflow.base` o similar— siguen siendo válidos con `git review
...
- [ ] T087 [P] `npm run preview --prefix vscode-extension`: agregar a
  ...
  (claro/oscuro/alto contraste) que los banners nuevos no dependen sólo de
  color y que los botones son `<button>` reales en orden de tab (FR-031,
  heredado de `002`).
- [ ] T088 `./tests/sandbox.sh` completo + recorrido manual de quickstart.md §
  1 a § 5 contra el sandbox real, no sólo los tests automáticos.
- [ ] T088a Auditoría de SC-004 y SC-005, que la spec define como "una revisión
  del código lo verifica" y que hasta acá no tenía quién la ejecutara: recorrer
  el código de la extensión contra
  [contracts/cli-invocation.md](./contracts/cli-invocation.md) confirmando (a)
  que toda invocación está en la lista, **verbo y argumentos** — el gate nuevo
  de FR-002, más estricto que el de `002`; (b) que ninguna acción que cambia de
  rama, mueve refs o descarta trabajo llega a invocarse sin confirmación previa;
  y (c) que ninguna de las prohibiciones de la tabla aparece en el código,
  incluida la fila nueva sobre enumerar ramas. Dejar constancia de qué se
  revisó, como hizo `004` con su verificación de la landing.
- [ ] T088b Validar SC-008 y el edge case de interrupción: matar el editor (o el
  proceso de la extensión) mientras corre un `finish` sobre una review con
  ediciones, reabrirlo, y confirmar que el panel **describe** el estado en el
  que quedó el repositorio — sea review normal, cierre pendiente o cierre
  trabado — en vez de mostrar un estado vacío o un error genérico. Repetir con
...
- **Foundational (Phase 2)**: depende de Setup. Bloquea la *implementación* de
  todas las historias (no sus tests, salvo donde se marca explícitamente).
  Dentro de la fase, T008 va antes que T008a (el flag apunta a un script que
  tiene que existir).
- **US1 (Phase 3)**: depende de Foundational. Es el MVP — ninguna otra historia
  ...
  del reporte de configuración que T024, T025 y T026 consumen: sin ella el
  estado vacío no tiene con qué decir contra qué se compararía.
...
- **US4 (Phase 6)**: depende de US3 completa (T044-T049) — sin el registro
  `finish` no hay estado sobre el que actuar.
- **US5 (Phase 7)**: depende de Foundational únicamente. Independiente de
  US2/US3/US4.
- **US6 (Phase 8)**: depende de US1 completa (T017-T030) — extiende el mismo
...
- T002-T004 (tests foundational) en paralelo entre sí, antes de T005-T008a.
- Con Foundational hecho: US1, US2, US3 y US5 pueden implementarse en paralelo
  por personas distintas — no comparten archivo salvo `extension.ts` y
  `package.json` (comandos), donde conviene coordinar el orden de merge.
- T010-T016 (tests de US1) en paralelo entre sí, antes de T017-T024a. **T028 no
  es paralelizable con T029**: necesita su fixture.
- T038-T043 (tests de US3) en paralelo entre sí, antes de T044-T052. **T037 no
  es paralelizable con T038**: necesita su fixture de conflicto, que es la
  pieza más cara de la feature y la que todos los demás reusan.
- T055-T056 (tests de US4) en paralelo, después de que US3 esté mergeada.
- T062-T063 (tests de US5) en paralelo, en cualquier momento después de
  Foundational.
- T068-T071 (tests de US6) en paralelo, después de que US1 esté mergeada.
- T081-T083 (los tres README) en paralelo entre sí.
...
Task: "Traducción de ReviewIntent a argv en reviewIntent.spec.ts"
```
...
2. Phase 2: Foundational (T002-T009) — bloquea la implementación, no los tests.
3. Phase 3: User Story 1 completa (T010-T030).
4. **Parar y validar**: quickstart.md § 2 contra el sandbox.
5. Esto ya es demostrable de punta a punta: desde el estado vacío hasta una
   review activa, sin la terminal.

### Incremental Delivery

1. Setup + Foundational → infraestructura compartida lista.
2. US1 → se puede iniciar desde el panel → validar → MVP.
3. US2 → se puede cancelar → validar independientemente.
4. US3 → se puede cerrar, y el panel deja de mentir sobre el cierre pendiente →
   validar.
5. US4 → el cierre pendiente y el trabado dejan de ser callejones → validar
   (depende de US3 mergeada).
6. US5 → se puede pausar, cerrando el ciclo con `continue` (ya existente) →
   validar.
7. US6 → el asistente gana rango y origen → validar (depende de US1 mergeada).
8. Polish → sandbox completo, documentación, suites completas.

### Parallel Team Strategy

Con varias personas, después de Foundational:

- Persona A: US1 (Phase 3) → luego US6 (Phase 8), que depende de su propio US1.
- Persona B: US3 (Phase 5) → luego US4 (Phase 6), que depende de su propio US3.
- Persona C: US2 (Phase 4) y US5 (Phase 7) en paralelo entre sí, sin depender
  de las otras dos personas.
- Todas coordinan el orden de merge de `extension.ts`/`package.json`, el único
...
- El verbo `config` (T017-T021) y el asistente de inicio (T024) son la pieza
  más grande de la feature y la que más historias tocan indirectamente (US1 y
  US6 la extienden, US3-US5 sólo dependen de la Fase 2). Conviene que sea de lo
...
- Confirmar que cada test nuevo falla antes de la implementación
  ...
  antes de T072-T074).
- **La regla que más fácil se pierde en la implementación** es la de no parsear
  la salida humana. El punto donde la tentación es máxima es T050: la CLI dice
  con todas las letras "no review changes to apply" y leer esa línea sería lo
  más corto. No se hace — se mira el estado posterior. T050a existe para que eso
  quede afirmado por un test, no por una nota.
- No hacer commit por tarea suelta si eso rompe `set -eu`/`shellcheck` o
  `tsc --noEmit` a mitad de camino: agrupar según haga falta para no dejar el
  checkout roto en ningún commit — la regla dura, no la granularidad de la
  lista.
- `--force` (T057) es la única acción de todo el ciclo que nunca aparece como
  opción de primera clase: si en algún momento de la implementación aparece
  como casilla o botón del primer diálogo, es una señal de que FR-021 se está
  perdiendo.
