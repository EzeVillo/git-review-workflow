#!/usr/bin/env bats
#
# Tests for review status, review prev, review abort and review finish --resume.
...
@test "review abort tears down a review already passed through review finish" {
	# Off-book path: review finish, then switch back to the still-present review
...
@test "review finish surfaces replay conflicts and resumes after resolution" {
	git review start feature/x --step
	printf 'FIX1\n' >f.txt
	git review next
	printf 'FIX2\nextra\n' >f.txt
...
@test "review finish --resume without a pending conflict fails" {
	git review start feature/x --step
	run git review finish --resume
	[ "$status" -ne 0 ]
